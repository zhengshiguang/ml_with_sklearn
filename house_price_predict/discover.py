...
def data_clean(data):
	...
	print(housing_extra_attrs)
	'''
	# Chapter 4: Feature Scaling
	# option 1: MinMaxScaler
	from sklearn.preprocessing import MinMaxScaler
	from sklearn.preprocessing import Imputer
	copy_data = data.drop(['ocean_proximity'], axis=1)
	imputer = Imputer(strategy='median', missing_values=np.nan)
	imputerd = pd.DataFrame(imputer.fit_transform(copy_data))
	imputerd.index = copy_data.index
	imputerd.columns = copy_data.columns
	#print(copy_data[imputerd.total_bedrooms.isnull()].head())
	min_max_scaler = MinMaxScaler()
	scaled = min_max_scaler.fit_transform(imputerd)
	print(scaled[0:10, ])
